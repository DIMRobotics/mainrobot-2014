...
odetect_set_single_limit(ODETECT_FULLFRONT, ODETECT_FRONT_DISTANCE);
/*!path: 384.187 */
chassis_move(100 , 100 , 1, 7, 8382 + mmToTicks(30));
GPIO_WRITE_HIGH(CONFIG_ROBOT_VACUUM);
tmgr_delay(100);
elevator_reset();
while (chassis_busy());
...
odetect_set_single_limit(ODETECT_RIGHT, ODETECT_TURN_DISTANCE);
chassis_move(-100, 100, 1, 10, 4732);
servo_write(2, 20);
tmgr_delay(400);
elevator_reset();
servo_write(1, 150);
...
/*!path: 1283.62 */
chassis_move(100 , 100 , 1, 7, 28006 + mmToTicks(30));
tmgr_delay(300);
GPIO_WRITE_HIGH(CONFIG_ROBOT_VACUUM);
tmgr_delay(100);
elevator_reset();
...
odetect_set_single_limit(0, 0);
/*!path: 117.158 */
chassis_move(60 , 60 , 1, 7, 2256);
elevator_reset();
elevator_set_pos(2050);
while (chassis_busy());
...
chassis_move(40, 40, 1, 7, mmToTicks(40));
elevator_set_pos(4800);
while (chassis_busy());
...
odetect_set_single_limit(ODETECT_FULLFRONT, ODETECT_FRONT_DISTANCE);
chassis_move(100, 100, 1, 7, mmToTicks(360));
while (chassis_busy());
...
odetect_set_single_limit(ODETECT_REAR, ODETECT_REAR_DISTANCE);
chassis_move(-80, -80, 1, 7, mmToTicks(130));
servo_write(1, 170);
elevator_reset();
while (chassis_busy());
...
odetect_set_single_limit(ODETECT_RIGHT, ODETECT_TURN_DISTANCE);
chassis_move(-100, 100, 1, 7, 5062);
while (chassis_busy());


odetect_set_single_limit(0, 0);
chassis_move(60, 60, 1, 7, mmToTicks(450));
servo_write(2, 27);
tmgr_delay(800);
elevator_reset();
while (chassis_busy());
...
tmgr_delay(800);
elevator_reset();
chassis_move(60, 60, 1, 7, mmToTicks(200));
while (chassis_busy());
*/
...
servo_write(2, 27);
odetect_set_single_limit(0, 0);
chassis_move(-30, -30, 1, 7, mmToTicks(20));
while (chassis_busy());
...
chassis_move(20, 20, 1, 10, mmToTicks(135));
servo_write(1, 63);
servo_write(2, 27);
tmgr_delay(800);
while (chassis_busy());
